...
const NewsPage: InferGetServerSidePropsType<typeof getServerSideProps> = (props: any) => {
  ...
  return (
    <>
      ...
      <PageHeader title={t(`news`)} image="https://d3leeb4r1qy96s.cloudfront.net/assets/img/cta-banner-image-1536x306.jpg"/>
        ...
        <main className="page-content col-md-9 col-md-push-3">
            ...
            <div id={index} key={index} className="post">
                <h3 className="post__title">
                  <Link href={`/news/${item.id}`} passHref>{locale === "mn" ? item.title : item.title_en}</Link>
                </h3>
                ...
                <div className="post__entry min-h-[200px]">
                    ...
                    <p>
                        <a className="button button--primary" href={`/news/${item.id}`} title={locale === "mn" ? item.title : item.title_en}> {t("learnmore")} </a>
                    </p>
                </div>
            </div>
            ))}
