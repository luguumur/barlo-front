...
const FormField = ({ icons = [], ...props }: Props) => {
  ...
  const controlClassName = [
    'py-2 pr-4 border-[#e2e2e2] border-x-0 border-t-0 w-full dark:placeholder-gray-400 leading-[18px] text-left',
    'focus:ring focus:ring-indigo-100 focus:border-indigo-200 focus:outline-none text-sm w-[274px]',
    ...
    <div className="mb-6 last:mb-0">
      {props.label && (
        <label
          ...
          className={`block mb-2 ${props.labelFor ? 'cursor-pointer' : ''}`}
        >
          {props.label}
        </label>
      )}
